...
import ConfirmationDialog from '@/components/dialogs/CourierFormDialog'
import {
...
export const AttemptsMap = {
  oneTime: { text: 'One Time' },
  twoTimes: { text: 'Two Times' },
  threeTimes: { text: 'Three Times' },
  fiveTimes: { text: 'Five Times' },
  TenTimes: { text: 'Ten Times' },
}
...
const AttemptsListTable = ({
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({row}) => {
          return (
            <Typography className='font-medium'>{row.original?.attempts || '—'}</Typography>
          )
        }
      },
      {
  ...
  const AttemptsArray = Object.keys(AttemptsMap).map(key => ({
    value: key,
    label: AttemptsMap[key].text
  }))
    ...
    <Card>
      ...
      <CardContent className='grid grid-cols-4 gap-3 '>
        <Autocomplete
          ...
          renderInput={params => (
            <TextField {...params} fullWidth placeholder='Attempts' label='Attempts' size='medium' />
          )}
        />
        <Autocomplete
          multiple
...
export default AttemptsListTable
