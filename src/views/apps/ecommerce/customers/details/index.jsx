...
import { fetchOrders, selectCustomerById, handleFindCustomer } from '@/redux-store/slices/order'
...
// const AddressBillingTab = dynamic(() => import('./customer-right/address-billing'), {
//   ssr: false,
//   loading: () => <div>Loading address and billing...</div>
// })
...
const CustomerDetails = ({ customerId }) => {
  ...
  // Memoized customer data to prevent unnecessary re-renders
  const customerData = useMemo(() => {
    if (!customerId || !orders.length) return null

    const orderWithCustomer = orders.find(order => order.customerData?.id == customerId)

    return orderWithCustomer?.customerData || null
  }, [customerId, orders])
  ...
  useEffect(() => {
    if (!hasFetched && customerId) {
      // Fetch orders with customer filter
      dispatch(
        fetchOrders({
