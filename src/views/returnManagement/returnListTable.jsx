...
export const statusChipColorForReturnManagement = {
  returned: { color: 'error', text: 'Returned' },
  damaged: { color: 'warning', text: 'Damaged' }
}

export const orderStatusArray = Object.keys(statusChipColorForReturnManagement).map(key => ({
  value: key,
  label: statusChipColorForReturnManagement[key].text
}))
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className='flex items-center gap-2'>
              <Chip variant='tonal' color={isPending ? 'warning' : 'success'} label={row.original.payment || '—'}/>
            </div>
          )
        }
      },
  ...
  return (
    <Card>
      <CardContent className='w-full flex items-start justify-between'>
        <div className='flex flex-wrap items-center gap-4 w-full'>
          ...
          <DebouncedInput
            ...
            placeholder='Search in Return orders'
            debounce={1000}
          />
      ...
      <CardContent className='flex justify-end gap-3'>
        <DialogActions className='px-1 py-0'>
          <div className='flex'>
            <Button
              onClick={() => {
